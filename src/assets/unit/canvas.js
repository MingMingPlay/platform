...
function CanvasAnimate(Dom,options){
    ...
    this.RoundColor = options.RoundColor || "#2592e7"
    this.RoundDiameter = options.RoundDiameter || 15
...
CanvasAnimate.prototype.getLength=function(){
    ...
    for(let i=0;i< this.length ;i++){
        ...
        obj.al = RandomNumBoth(2,6)/10;
        obj.controlX = parseInt( Math.random()*40 ) > 20 ? "left":"right"
        obj.controlY = parseInt( Math.random()*40 ) > 20 ? "bottom":"top"
        arr.push(obj)
    }
    return arr
}
...
CanvasAnimate.prototype.drawRound = function(obj){
    ...
    this.Dom.globalAlpha = al;
    this.Dom.shadowBlur = 20;
    this.Dom.shadowColor = "#fff";
    this.Dom.fill()
    this.Dom.closePath()
}
...
CanvasAnimate.prototype.Clicked = function(event){
    ...
    obj.y = event.clientY
    obj.r = parseInt( Math.random()*10 )
    obj.controlX = parseInt( Math.random()*29 ) > 5 ? 'left' :'right'
