<template>
    <div class="incentive">
            ...
            <el-form :inline="true" :model="form">
                <el-form-item label="方案类型">
                    <el-select  v-model="form.type" clearable placeholder="请选择">
                        <el-option :key="0" label="单实物激励" value="SingleEntity"></el-option>
                        <el-option :key="1" label="多实物激励" value="MultiEntity"></el-option>
                        <el-option :key="2" label="单旅游激励" value="SingleTravel"></el-option>
                        <el-option :key="3" label="多旅游激励" value="MultiTravel"></el-option>
                        <el-option :key="4" label="现金激励" value="Cash"></el-option>
                        <el-option :key="5" label="加佣" value="Commission"></el-option>
                        <el-option :key="6" label="其他激励" value="Other"></el-option>
                    </el-select>
                </el-form-item>
        ...
        <el-table :data="tableData" v-loading.body="loading" stripe>
            <el-table-column type="index" label="ID" width="64" align="center"></el-table-column>
            <el-table-column label="方案名称" prop="title" align="center"></el-table-column>
            <el-table-column label="所属机构" prop="publishMerchantName" align="center"></el-table-column>
            ...
            <el-table-column label="操作" align="center">
                <template slot-scope="scope">
                    <el-button type="text" @click="handleLookDetail(scope.row.sid)" v-if="scope.row.state !== 'Inactive'">查看</el-button>
                    <el-button type="text" @click="handleUpdateIncentive(scope.row.sid)" v-if="scope.row.state === 'Inactive' || scope.row.state==='Permit'">修改</el-button>
                    <el-button type="text" @click="handleDeleteIncentive(scope.row.sid)" v-if="scope.row.state === 'Inactive'">删除</el-button>
                    <el-button type="text" @click="handlePublishIncentive(scope.row.sid)" v-if="scope.row.state==='Permit'">发布</el-button>
                    <el-button v-if="scope.row.state === 'Active' || scope.row.state === 'Underway'  " type="text" @click="handleNullify(scope.row.sid)">作废</el-button>
                </template>
            </el-table-column>
        </el-table>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            //删除
            handleDeleteIncentive(sid){
                pubObj.showMessage(this, {
                    title: '删除方案',
                    content: '您确定要删除该方案？',
                    confirm() {
                        pubObj.httpRequest(this, {
                            url: ['incentive', 'delete/', sid],
                            method: 'POST',
                            success(result) {
