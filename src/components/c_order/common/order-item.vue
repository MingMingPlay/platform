<template>
    <div class="order-item">
        <p class="item-header">
            ...
            <template v-if="orderName === 'special'">
                <span>供货商：{{ orderData.merchantName }}</span>
                <span>代理商：{{ orderData.agentName }}</span>
            </template>
            <span>下单时间：{{ orderData.createdTime }}</span>
            <span>买家：{{ orderData.userName }}</span>
            <span>来源：{{ orderData.channelName }}</span>
            <span v-if="orderName === 'self' || orderType === 'se'">商家名称：{{ orderData.merchantName }}</span>
        </p>
        <el-table :data="[orderData]" border>
            ...
            <!--<el-table-column
                label="商家名称"
                prop="merchantName"
                v-if="orderName === 'self' || orderType === 'se'">
            </el-table-column>-->
            ...
            <el-table-column label="操作" v-if="!isDetail">
                <template slot-scope="scope">
                    ...
                    <template v-if="orderType!='su' && orderName=='refund' && scope.row.state==1">
                        <el-button
                            type="text"
                            class="vertical-center"
...
//  import api from '../../../assets/config/c-api.js'
    ...
    export default {
        ...
        computed: {
            orderData() { return this.orderItemData; }
        },
        ...
        methods: {
            handleEnterDetail(row) {
                ...
                this.$router.push({
                    path: '/c_order/'+ this.orderName +'/detail',
                    query: { orderSid: row.sid, orderState: row.state }
                })
            },
            // 点击确认退款
            handleConfirm(row) {
                bus.$emit('open-confirm-dialog', row)
            },
            // 拒绝退款
            handleRefuse(orderId) {
                pubObj.showMessage(this, {
                    title: '拒绝退款',
                    content: '您确定拒绝退款给该用户吗?',
                    confirm() {
                        pubObj.httpRequest(this, {
                            method: 'POST',
                            url: this.$api.order + 'refund/disagree/',
