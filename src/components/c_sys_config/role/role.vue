...
export default {
	...
	methods: {
		getData (param){		// 获取列表
			...
			this.$http.get(['roleCloud'], {params: param})
		...
		addRoleConfirm (){		// 新增和修改角色
			this.$refs.form.validate((valid) => {
				if(valid){
					var url = ['roleCloud', this.dialog.type === 'PUT'?this.form.sid:''];
					this.$http({
		...
		delRole (sid){		// 删除角色
			...
			.then(() => {
				this.$http.delete(['roleCloud', sid]).then(function(result){
					if(result.body.code === 200){
						this.getData();
						this.$message({type:'success',message:result.body.message})
					}else {
		...
		getRoleAuth (sid){		// 根据角色获取权限
			...
			this.$http.get(['roleCloud', sid + '/perm'])
			.then(function(result){
				if(result.body.code === 200){
		...
		saveAuth (){		// 保存修改权限
			...
			this.$http.put(['roleCloud', this.tempSid + '/perm'], this.authForm)
			.then(function(result){
		...
		editUserAuth (sid){		// 根据角色sid获取用户列表
			...
			this.tableLoading = true;
			this.$http.get(['roleCloud', sid + '/user'])
			.then(function(result){
				if(result.body.code === 200){
					this.tableLoading = false;
		...
		saveAuthUser (){		// 保存授权用户
			...
			this.$http.put(['roleCloud', this.tempSid + '/user'], this.authUserForm)
			.then(function(result){
