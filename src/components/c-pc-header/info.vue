			...
			<div class="detail-wrap">
				...
				<el-form-item label="区域" class="overflow">
					<el-select v-model="proName" placeholder="省" @change="handleChangePro" clearable>
						<el-option v-for="item in pro" :key="$index" :label="item.name" :value="item.sid"></el-option>
					</el-select>
					<el-select v-model="cityName" placeholder="市" @change="handleChangeCity" clearable>
						<el-option v-for="item in city" :key="$index" :label="item.name" :value="item.sid"></el-option>
					</el-select>
					<el-select v-model="areaName" placeholder="区" @change="handleChangeArea" clearable>
						<el-option v-for="item in area" :key="$index" :label="item.name" :value="item.sid"></el-option>
					</el-select>
				</el-form-item>
...
<style>
	.account-info .el-form-item__content {
		width: 400px;
	}

	.account-info .el-select {
		width: 50%;
	}

	.account-info .overflow .el-select {
		width: 32%;
		float: left;
	}

	.account-info .el-form-item__content .el-select+.el-select {
		margin-left: 2%;
	}
