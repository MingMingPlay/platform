<template>
    <div class="transaction">
        ...
        <div class="search-fil-wrap one-line">
            <el-form class="search-form" :model="searchForm" :inline="true">
                <el-form-item label="订单号">
                    <el-input v-model="searchForm.keyword" placeholder="系统订单号/支付单号"></el-input>
                </el-form-item>
        ...
        <el-table :data="data" :stripe="true" v-loading="loading">
            ...
            <el-table-column prop="createdTime" label="创建时间" width="150px" label-class-name="first-col-no-padding" class-name="first-col-no-padding"></el-table-column>
            <el-table-column label="系统订单号/支付流水号" width="230px">
                <template slot-scope="scope">
                    <p>{{scope.row.sysOrderId}}</p><p>{{scope.row.paymentId}}</p>
                </template>
            </el-table-column>
            ...
            <el-table-column prop="terminal" label="支付端" align="center"></el-table-column>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getData(param){    // 获取用户交易流水
                this.loading = true;
                this.$http.get(`${api.cWalletUser}consume`, {params: param})
                    .then(result => {
                        this.loading = false;
                        if(result.body.code === 200){
