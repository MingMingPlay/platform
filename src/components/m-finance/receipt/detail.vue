<template>
    <div>
        ...
        <div class="detail-title">收款单详情信息</div>
        <div class="search-fil-wrap one-line receiptDetail">
            <ul>
                <li><span>收款单编号：</span><span>{{Detail.settlementId}}</span></li>
                <li><span>收款方式：</span><span>{{Detail.payModeName}}</span></li>
                <li><span>本期应收：</span><span>{{Detail.totalAmountValue | kb}}</span></li>
            </ul>
            <ul>
                ...
                <li><span>收款账号：x</span><span>{{Detail.bankAccountNo}}</span></li>
            </ul>
            <ul>
        ...
        <el-table
            ...
            <el-table-column v-for="(column, index) in columns" :key="column.dataIndex" :label="column.text" align="center">
                <template slot-scope="scope">
                    <span
                        v-if="column.dataIndex=='actualAmountValue' && scope.row['actualAmountValue']">
                        {{scope.row['actualAmountValue'] | kb}}
                    </span>
...
<script>
    ...
    import { receiptDetail } from '../../../assets/staticData/finance.js'
    ...
    export default {
        ...
        data() {
            return {
                ...
                isAuditShow:false,
                //表单头部
                columns: receiptDetail.tableColumns,
                //表单详情数据
                tableData: [],
                //面包屑
                crumbs:receiptDetail.crumbs,
                currentPage4: 1,
                //时间值
                createTime:[],
                //订单号
        ...
        methods: {
            ...
            getData(){
                this.loading = true;
                this.$http.get(['financialGatherings'],{
                    params:this.form
                })
                    .then((result) => {
