            ...
            <el-form :model="shippingForm" :rules="rules" ref="shippingForm" label-width="80px" style="margin-top: 18px;">
                ...
                <el-form-item label="区域地址" prop="addressSid">
                    <el-select class="area-full" v-model="shippingForm.addressSid" placeholder="请选择...">
                        <el-option v-for="(item,index) in addressData" :key="index" 
                            :label="item.label" :value="item.value"></el-option>
                    </el-select>
                </el-form-item>
                <el-form-item label="发货时间" prop="deadline">
                    <el-select v-model="shippingForm.deadline" placeholder="请选择...">
                        <el-option v-for="(item,index) in staticData.deadline"
                            :key="index" 
                ...
                <el-form-item label="是否包邮" prop="shippingFree">
                    <el-radio-group ref="shippingFree" v-model="shippingForm.shippingFree">
                        <el-radio :label="false" @change.native="handleShippingNotFree">不包邮</el-radio>
                        <el-radio :label="true" @change.native="handleShippingFree">包邮</el-radio>
                    </el-radio-group>
                </el-form-item>
                                ...
                                <el-table-column label="运送方式" prop="shippingTypeName" width="100">
                                    <template slot-scope="scope">
                                        <el-select v-model="scope.row.shippingType">
                                            <el-option v-for="(item,index) in checkedListName"
                                                :key="index" 
...
<script>
    ...
    export default {
        ...
        mounted() {
            // 模板id
            this.templateSid = this.$route.query.tp_sid;
            pubObj.httpRequest(this, {
                url: api.shipping + this.templateSid,
                method: 'GET',
                success(result) {
                    this.shippingForm = result.data.templateVO;
                    this.initShippingType(this.shippingForm.shippingType);
                    const free = this.shippingForm.shippingFree;
                    // 判断是否包邮
                    if(free) {
                        this.isShippingFree = free;
                    } else {
                        this.initTemplate(result.data.policyVOList);
                        this.FreeData = result.data.freePolicyVOList;
                        if(this.FreeData.length > 0) this.checked2 = true;
                        // 组装已经选择的城市
                        setTimeout(() => {
                            this.ExpressChecked = this.ExpressData.reduce((arr, item) => {
                                arr = arr.concat(item.areaIds.split(','));
                                return arr;
                            }, []);
                            this.EMSChecked = this.EMSData.reduce((arr, item) => {
                                arr = arr.concat(item.areaIds.split(','));
                                return arr;
                            }, []);
                            this.FreeChecked = this.FreeData.reduce((arr, item) => {
                                arr = arr.concat(item.areaIds.split(','));
                                return arr;
                            }, []);
                        }, 50)
                    ...
                    /* 监听改变 */
                    /*this.$refs.shippingFree.$on('change', (bool) => { console.log(bool) })*/
                }
            });
            this.getAddress();
        },
        ...
        methods: {
            // 初始化ShippingType
            initShippingType(type) {
                if(type === 'ExpressAndEMS') this.checkedList.push('Express', 'EMS');
                else this.checkedList.push(type);
            },
            // 包邮
            handleShippingFree(e) {
                ...
                if(bool) {
                    pubObj.showMessage(this, {
                        ...
                        cancel() { this.shippingForm.shippingFree = false; }
                    });
                } else this.isShippingFree = bool;
            },
            // 不包邮
            handleShippingNotFree(e) {
                const bool = e.target.checked;
                if(bool) this.isShippingFree = false;
            },
            // 中间方法获取当前行
            ...
            // 拆分策略
            sliceTemplate(name, policyData) {
                this[name + 'Data'] = policyData.filter((item) => {
                    return item.shippingType === name;
                }).filter((item) => {
                    if(!item.policyDefault) return true;
                    else {
            ...
            // 初始化展示模板策略
            initTemplate(policyData) {
                this.sliceTemplate('Express', policyData);
                this.sliceTemplate('EMS', policyData);
            },
            // 地址库
            getAddress() {
                pubObj.httpRequest(this, {
            ...
            validDot(name, key) {
                ...
                const [a, b] = [obj[key].toString().indexOf('.'), obj[key].toString().indexOf('-')];
...
<style>
    .shipping-edit .el-form-item { width: 800px; }
    .shipping-edit .el-cascader__label { line-height: 28px; }
    .shipping-edit .rule-wrapper { padding: 5px; border: 1px solid #ddd; }
    .shipping-edit .default-rule { background-color: #E8F2FF; padding: 5px; }
    .shipping-edit .default-rule .el-input { width: 100px; }
    .shipping-edit .customize-rule .el-table .cell { padding: 0 8px; }
    .shipping-edit .customize-rule .el-table .cell .el-select .el-input__icon { width: 20px; }
    .shipping-edit .customize-rule .el-table .cell .el-select .el-input__inner { padding-right: 20px; }
    .shipping-edit .shipping-type .el-checkbox { display: block; }
    .shipping-edit .area-full { width: 80%; }
    .shipping-edit .item-error { color: #ff4949; font-size: 12px; line-height: 1; padding: 6px 0; }
</style>
