        ...
        <el-form :model="dialogForm" :rules="rules" ref="dialogForm" label-width="90px">
           ...
           <el-form-item label="联系电话" prop="phone">
            ...
            <el-form-item label="微信二维码">
                <image-upload 
                    v-model="dialogForm.wxImageSid"
                    action-type="NameCard"
                    :action-path="dialogForm.wxImagePath">
                </image-upload>
            </el-form-item>
        </el-form>
...
<script>
    ...
    export default {
        ...
        created() { this.dialogForm.labels = [null, null] },
        methods: {
            // 自定义拦截器（处理荣誉标签）
            interceptor(data) {
                let labels = data.labels;
                if(!labels || labels.length === 0) data.labels = [null, null];
                else if(labels.length === 1) data.labels.push(null);
                return data;
            },
            // 获取详情数据
            getData() {
            ...
            handleCancel() { this.$emit('update:visible', false); },
