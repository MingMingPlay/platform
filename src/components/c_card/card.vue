<template>
	<div class="card">
		...
		<pagination :event-name="eventName" :total="pagiTotal"></pagination>
		...
		<div class="template" v-if="templateVisible" @click="templateVisible = false">
            <iframe :src="templatePath" frameborder="0" class="iframe" id="parDiv" width="300px" height="450px"></iframe>
        </div>
...
<script>
	import { new_pagination } from '../common/jkc-mixin.vue'
	...
	export default {
		...
		mounted() {
			...
			this.$watch('form.state', (newVal, oldVal) => {
				...
				// 查询
				bus.$emit(this.eventName, { pageIndex: 1, pageSize: 10 });
			})
		},
		methods: {
		    // 修改成功之后通知父级，刷新列表
...
//          },
			...
			handleLookCard(sid, state) {
			    if(state === 'Used') {
			        ...
			        /*this.templatePath = 'http://192.168.0.180:8020/card/shareTest.html?sid=' + sid;*/
			...
			handleLookFans(sid, state) {
				router.push({ path: '/c_card/fans', query: { sid: sid, state: state } });
			}
		}
	}
</script>
