    ...
    <div class="subscribe-order">
        ...
        <div class="state-btn-wrapper">
            ...
            <jk-tabs-nav v-model="queryObject.state" @change="handleStateChange">
                ...
                <jk-tabs-pane :label="3">待收货</jk-tabs-pane>
            ...
            <el-button 
                ...
                @click="handleExportExcel($api.order + 'subscribe/excel', '征订商品订单')">导出订单Excel
            </el-button>
        ...
        <pagination 
            event-name="subscribe-pagination" 
            :pagination="pagination"
            :total="pagiTotal"
            @subscribe-pagination="handlePagination">
        </pagination>
...
<script>
    ...
    export default {
        ...
        created() {
            this.crumbs = [{ title: '订单管理' }, { title: '征订商品订单' }];
        ...
        methods: {
            ...
            getAwaitingOrder() {
                pubObj.httpRequest(this, {
                    url: api.order + 'subscribe?state=2',
                    success(result) {
...
//              window.open(encodeURI(api.order + 'subscribe/excel?' + queryString), '_blank');
//          },
            // 发货成功之后, 刷新数据
            handleRefreshData() {
                this.getTableList(this.queryObject);
...
<style>
    .subscribe-order .state-btn-wrapper { position: relative; margin-bottom: 18px; }
    .subscribe-order .export-order-btn {
        position: absolute;
        top: 4px;
        right: 20px;
        z-index: 999;
