	...
	<div class="goods-add goods-supply-add">
		...
		<div v-show="stepsActive === 0">
					...
					<div class="cate">
						<el-form ref="basisForm5" :model="goodsBasis" :rules="rules">
							<el-form-item required prop="agentShopSid">
								<el-select v-model="goodsBasis.agentShopSid" placeholder="请选择代理店铺" no-data-text="无代理店铺">
									<el-option
			...
			<div class="cate-wrap">
				...
				<div class="cate-cell clear">
					<div class="cate-subtitle">
						<p class="cell-title"><span>运费</span></p>
					</div>
					<div class="cate">
						<!--<el-form ref="basisForm6" :model="goodsBasis" :rules="rules">-->
						<el-form :model="goodsBasis" :rules="rules">
		...
		<div class="button-group" v-show="stepsActive == 0">
			<el-button type="info" v-show="brandData.length > 0 && agentData.length > 0" @click="nextStep" :loading="buttonLoading">下一步</el-button>
			<el-button type="info" v-if="agentData.length == 0" disabled>请先建立供应关系</el-button>
			<el-button type="info" v-else-if="brandData.length == 0" disabled>请联系平台管理员添加品牌</el-button>
		</div>
		<div class="button-group" v-show="stepsActive == 1">
			<el-button type="info" @click="lastStep">上一步</el-button>
			<el-button type="warning" @click="addSkuData('toaudit')" :loading="buttonLoading">提交审核</el-button>
...
<script>
    ...
    export default {
        ...
        created (){
            ...
            // 获取店铺信息拿到店铺sid，再获取运费模板
            this.$http.get(['merchantShopDetail', this.$root.userInfo.merchantSid])
                .then(result => {
                    if(result.body.code === 200){
                        this.getShipping(result.body.data.sid);
                    }
                })
        },
        methods: {
            getAgentData (sid){		// 获取代理商
                this.$http.get(['shopAgent'])
                    .then((result) => {
                        if(result.body.code === 200){
            ...
            getShipping (shopSid){		// 获取运费模板
                this.$http.get(api.shipping + 'list?pageSize=1000&shopSid=' + shopSid)
                    .then((result) => {
            ...
            selectSku (sid, data, currentItem){		// 选择系统的规格
                ...
                for(let i=0;i<this.selectTemp.length;i++){
                    ...
                    if((typeof value === 'string' && name === value) || (typeof value === 'object' && name === value.name)){
                        if(currentItem.sid === this.selectTemp[i].sid){
                            currentItem.parentAttrSwitch = false;
                        }else {
            ...
            selectAttrConfirm (sid, index, item, childrenItem){	// 选择系统规格值后确定或取消
                ...
                for(let i=0;i<temp.length;i++){
                    if((typeof data === 'string' && temp[i].name === data) || (typeof data === 'object' && temp[i].name === data.name)){
                        if(childrenItem.sid === temp[i].sid){
                            childrenItem.childrenAttrSwitch = false;
                        }else {
