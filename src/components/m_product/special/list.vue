<template>
    <div class="goods-assist">
        ...
        <!--<el-tabs v-model="userIdent" type="card" @tab-click="typeClick">-->
            <!--<el-tab-pane label="我供货的商品" name="ident0"></el-tab-pane>-->
            <!--<el-tab-pane label="我代理的商品" name="ident1"></el-tab-pane>-->
        <!--</el-tabs>-->
        <div class="search-fil-wrap">
            <el-form class="search-form" label-width="80px" :model="searchForm" :inline="true">
                ...
                <el-form-item label="供应店铺" v-if="userIdent == 'ident1'">
                    <el-select
                ...
                <el-form-item label="代理店铺" v-if="userIdent == 'ident0'">
                    <el-select
                        v-model="searchForm.agentShopSid"
                        clearable
                        filterable
                        remote
                        placeholder="选择代理店铺"
                        :remote-method="getAgentShop"
                        :loading="getLoading">
                        <el-option
        ...
        <div class="tab-wrap">
            <el-tabs v-model="goodsStates" @tab-click="tabClick">
                ...
                <el-tab-pane label="仓库中" name="Storage" v-if="userIdent === 'ident0' && $root.userInfo.merchantSid !== 'ooo_PJ'"></el-tab-pane>
                <el-tab-pane label="仓库中" name="AStorage" v-if="userIdent === 'ident1' && $root.userInfo.merchantSid !== 'ooo_PJ'"></el-tab-pane>
                <el-tab-pane label="待审核" name="Check" v-if="$root.userInfo.merchantSid !== 'ooo_PJ'"></el-tab-pane>
                <el-tab-pane label="未通过" name="Refuse" v-if="$root.userInfo.merchantSid !== 'ooo_PJ'"></el-tab-pane>
            </el-tabs>
            <div class="sort-wrap">
                <el-button type="primary"
                           v-if="userIdent == 'ident0'"
                           @click="$router.push({path: '/m_product/special/add',query:{btn: $route.query.btn,step: 0}})">
                    发布行销商品
                </el-button>
            </div>
        </div>
        ...
        <goods-table :tableData="goodsList[goodsStates]"
                     :tableHeader="headerData[userIdent][goodsStates]"
                     :index="true"
                     :tableLoading="tLoading[goodsStates]"
                     v-if="tableShow">

            <!-- 供应商销售中操作 -->
            <el-table-column label="操作" width="180" align="right" slot="handle"
                             v-if="userIdent == 'ident0' && goodsStates == 'Selling'">
                <template slot-scope="scope">
                    <el-button type="text"
                               @click="offlineGoods(scope.row.sid)">下架</el-button>
                    <el-button type="text"
                               @click="$router.push({
                                   path: $route.path + '/info',
                                   query:{btn: $route.query.btn, agent: $route.query.agent, spuSid: scope.row.sid}
                               })">详情</el-button>
                </template>
            </el-table-column>
            ...
            <!-- 代理商销售中操作 -->
            <el-table-column label="操作" width="180" align="right" slot="handle"
                             v-else-if="userIdent === 'ident1' && goodsStates === 'Selling'">
                <template slot-scope="scope">
                    <!--<el-button type="text"-->
                               <!--@click="offlineGoods(scope.row.sid)">下架</el-button>-->
                    <el-button type="text"
            ...
            <!-- 供应商仓库中操作 -->
            <el-table-column label="操作" width="180" align="right" slot="handle"
                             v-else-if="userIdent == 'ident0' && goodsStates == 'Storage'">
                <template slot-scope="scope">
                    <template v-if="scope.row.inventory > 0 && (scope.row.state == 'Active' || scope.row.state == 'Offline')">
                        <el-button type="text" v-if="shopInfo.exemption"
                                   @click="onlineGoods(scope.row.sid)">上架</el-button>
                        <el-button type="text" v-else-if="shopInfo.exemption === false"
                                   @click="toauditGoods(scope.row.sid)">提交审核</el-button>
                    </template>
                    <!--<el-button type="text" v-if="scope.row.state == 'Active'"-->
                               <!--@click="toauditGoods(scope.row.sid)">提交审核</el-button>-->
                    <el-button type="text"
                               v-if="scope.row.state == 'Active' || scope.row.state == 'ProductDraft' || scope.row.state == 'Offline'"
                               @click="editGoods(scope.row.sid)">编辑</el-button>
                    <!--<el-button type="text"-->
                               <!--v-if="scope.row.state == 'Approval' || scope.row.state == 'Offline'"-->
                               <!--@click="$router.push({-->
                                   <!--path: $route.path + '/info',-->
                                   <!--query:{btn: $route.query.btn, agent: $route.query.agent, spuSid: scope.row.sid}-->
                               <!--})">详情</el-button>-->
                    <el-button type="text"
                               v-if="scope.row.state == 'Active' || scope.row.state == 'ProductDraft' || scope.row.state == 'Offline'"
                               @click="delGoods(scope.row.sid)">删除</el-button>
                </template>
            </el-table-column>

            <!-- 代理商仓库中操作 -->
            <el-table-column label="操作" width="180" align="right" slot="handle"
                             v-else-if="userIdent == 'ident1' && goodsStates == 'AStorage'">
                <template slot-scope="scope">
                    <el-button type="text"
                               v-if="scope.row.sellingPrice && scope.row.employeePrice && shopInfo.exemption && scope.row.inventory > 0"
                               @click="onlineGoods(scope.row.sid)">上架</el-button>
                    <el-button type="text"
                               v-else-if="scope.row.sellingPrice && shopInfo.exemption == false"
                               @click="toauditGoods(scope.row.sid)">提交审核</el-button>
            ...
            <!-- 代理商待审核操作 -->
            <el-table-column label="操作" width="180" align="right" slot="handle"
                             v-else-if="userIdent == 'ident1' && goodsStates == 'Check'">
                <template slot-scope="scope">
                    <el-button type="text"
                               v-if="scope.row.state == 'AgentCheck'"
                               @click="$router.push({
                                   path: $route.path + '/info',
                                   query:{btn: $route.query.btn, agent: $route.query.agent, spuSid: scope.row.sid}
                               })">审核商品</el-button>
            ...
            <!-- 供应商未通过操作 -->
            <el-table-column label="操作" width="180" align="right" slot="handle"
                             v-else-if="userIdent == 'ident0' && goodsStates == 'Refuse'">
                <template slot-scope="scope">
                    <el-button type="text"
                               v-if="scope.row.state == 'AgentRefuse' || scope.row.state == 'CloudRefuse'"
                               @click="toauditGoods(scope.row.sid)">提交审核</el-button>
                    <el-button type="text"
                               @click="editGoods(scope.row.sid)"
                               v-if="scope.row.state == 'AgentRefuse' || scope.row.state == 'CloudRefuse'">
                        编辑
                    </el-button>
                    <el-button type="text"
                               @click="delGoods(scope.row.sid)"
                               v-if="scope.row.state == 'AgentRefuse' || scope.row.state == 'CloudRefuse'">
                        删除
                    </el-button>
                </template>
            ...
            <!-- 代理商未通过操作 -->
            <el-table-column label="操作" width="180" align="right" slot="handle"
                             v-else-if="userIdent == 'ident1' && goodsStates == 'Refuse'">
                <template slot-scope="scope">
                    <el-button type="text"
                               v-if="scope.row.state == 'CloudRefuse'"
                               @click="toauditGoods(scope.row.sid)">提交审核</el-button>
                    <el-button type="text"
                               @click="editGoods(scope.row.sid)"
                               v-if="scope.row.state == 'CloudRefuse'">
    ...
    export default {
        data() {
            return {
                ...
                userIdent: 'ident' + this.$route.query.agent,       // 切换代理商品和供货商品，1代理的，0供货的
                searchForm: {
                    title: '',
                    brandSid: '',
                    start: '',
...
//                    purchase: 0,
                ...
                supplierData: [],
                agentData: [],
                getLoading: false,      // 远程获取供应商店铺或代理商店铺loading
        ...
        created (){
            ...
            if(this.$route.query.agent == undefined){
                // 如果是太平，只代理，ooo_PJ 是太平的 sid
                if(this.$root.userInfo.merchantSid === 'ooo_PJ'){
                    query.agent  = '1';
                    this.userIdent = 'ident1';
                }else {
                    query.agent  = '0';
                    this.userIdent = 'ident0';
                }
            }
        ...
        methods: {
            typeClick(tab, event) {     // 切换代理或供货的商品
                this.resetSearch();
                let query = JSON.parse(JSON.stringify(this.$route.query));
                if(tab.name == 'ident1'){
                    query.agent = '1';
                    this.searchForm.agentShopSid = '';
                }else {
            ...
            getSupplierShop(query){
                if(query === ''){
                    this.searchForm.shopSid = '';
                }
                if (query !== '' && query.length > 1){
                    this.remoteMethod(query, 'supplier');
                }else {
                    this.supplierData = [];
                }
            },
            getAgentShop(query){
                ...
                if (query !== '' && query.length > 1){
                    this.remoteMethod(query, 'agent');
                }else {
                    this.agentData = [];
                }
            },
            remoteMethod(query, type) {		// 动态搜索代理商信息
                let url = '';
                if(type === 'supplier'){
                    // 此处查除太平以外所有商家的店铺
                    url = ['merchantShopNTp'];
                ...
                this.$http.get(url, {params: {q: query}})
                    .then((result) => {
                        this.getLoading = false;
                        this[type + 'Data'] = result.body.data.filter(item => {
            ...
            getGoodsList (params){		// 获取商品列表
                ...
                this.$http.get(api.spu + 'merchant', {params : params})
                    .then((result) => {
            ...
            editGoods (sid){		// 编辑商品
                ...
                if(this.$route.query.agent == 1){
                    this.$router.push({path: '/m_product/special/add_agent',query:query})
                }else if(this.$route.query.agent == 0){
//                    this.$router.push({path: '/m_product/special/add_supply',query:query})
                    this.$router.push({path: '/m_product/special/add',query:query})
                }
            },
            delGoods (sid){			// 删除商品
...
<style>
    .goods-assist .search-fil-wrap .el-form--inline .el-form-item__content,.goods-assist .search-fil-wrap .el-select,.goods-assist .search-fil-wrap .el-date-editor--daterange.el-input{
        width: 210px;
        height: 28px;
    }
    .goods-assist .tab-wrap{
        position: relative;
    }
    .goods-assist .tab-wrap .sort-wrap{
        position: absolute;
        right: 0;
        top: 6px;
    }
    .goods-assist .tab-wrap .sort-wrap .el-button{
        margin-left: 10px;
    }
    .goods-assist .table-select-all{
        margin-bottom: 15px;
    }
    .goods-assist .table-select-all .el-checkbox{
        margin-right: 10px;
    }
    .goods-assist .goods-title{
        display: flex;
        padding: 5px 0;
    }
    .goods-assist .goods-title img{
        width:50px;
        height:50px;
        margin-right: 10px;
        border: 1px solid #e4eaee;
    }
    .goods-assist .goods-title .title{
        flex: 1;
        overflow: hidden;
    ...
    .goods-assist .pagination-wrap{
        margin-top: 20px;
        text-align: center;
    }
    .goods-assist .el-table .el-table__body .red{
        color: #f00;
    }
