...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            clearPurchase(){
                // 提交时如果没有设置加入集采商城，把集采价清空
                let ifPurchase = this.batchSkuData.some(value => {
                    if(value.name === '集采价'){
                        return true;
                    }
                    return false;
                ...
                if(!ifPurchase){
                    for(let key in this.attrSkuData){
                        this.attrSkuData[key].purchasePrice = '';
                    }
                }
            },
            submitData(type, callback){
                ...
                if(type && type !== 'save'){
                    ...
                    let sid = this.spuSid || this.$route.query.spuSid;
                    if(!dataChange){
                        url = this.$api.spu + 'audit/' + type + 'save' + '/' + sid;
                    }else {
                        url = this.$api.spu + 'audit/' + type + '/' + sid;
                    }
                }else {
                    url = this.$api.sku;
                    apiVer = '2.0.0';
