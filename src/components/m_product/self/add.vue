<template>
	<div class="goods-add">
		...
		<div v-show="stepsActive === 0">
			...
			<div class="cate-wrap">
				...
				<div class="cate-cell clear">
					...
					<div class="cate">
						<el-form ref="basisForm1" :model="goodsBasis" label-width="100px" :rules="rules">
					...
					<div class="cate">
						<el-form :model="goodsBasis">
							<el-form-item>
                                ...
                                <!--<span v-else>分　抵扣金额<i :loading="true">{{ScoreMoney}}</i>元　</span>-->
                                <span v-else>
                                    分　抵扣金额
                                ...
                                <el-checkbox
                                    ...
                                    :disabled="goodsBasis.ifPromotion"
                                    @change="calcScore"
                                    style="color: #f00;">加入积分商城</el-checkbox>
							</el-form-item>
		...
		<div class="button-group" v-show="stepsActive === 0">
			...
			<el-button type="info" v-show="brandData.length === 0" disabled>请联系平台管理员添加品牌</el-button>
		</div>
...
//				}
			...
			addSkuData (type){		// 新增或修改库存数据
				...
				for(let key in this.attrSkuData){
                    let typeSwitch1 = false;    // 两个字段一起实现当前行如果有任意一个必填项有值，那其他的也必须有值
                    let typeSwitch2 = false;    // 有值1变为true，没有值2变为true
				    ...
				    for(let i=0;i<this.batchSkuData.length;i++){
                        ...
                        if(pubObj.isEmpty(value) && value <= 0){
                            if(item.type === 'regularPrice' || item.type === 'sellingPrice'){
                                priceZero = true;
                            }
                        }
